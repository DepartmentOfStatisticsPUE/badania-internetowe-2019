...
X_2 <- rpois(n = N_pop, lambda = 0.5) ## liczba dzieci
X_3 <- rbinom(n = N_pop, size = 1, prob = 0.7) ## miejsce zamieszkania (1=wies, 0=miasto)
Y_1 <- 200 + 20*X_1 - 15*X_2 + 30*X_3 + rnorm(n = N_pop, mean = 0, sd = 10)
...
delta <- rbinom(n = N_pop, size = 1, 
                prob = exp(7 - 10*X_1 + 15*X_2 - 7*X_3) / (1 + exp(7 - 10*X_1 + 15*X_2 - 7*X_3)))
...
res_calib <- calibrate(big_data_svy, 
                       formula = list(~X_1 + X_3), 
                       population = list(pop_x1x3), 
                       calfun = cal.linear)
...
```{r}
m0 <- glm(flag ~ X_1 + X_3, data = df, family = binomial())
summary(m0)
```
